...
ganache-cli --gasLimit 50000001 --port 8546 --network developtment > ganache.log 2>&1 &
GANACHE_PID=$!
...
truffle migrate --network development --config truffle-config.cjs --reset --verbose-rpc
if [ $? -ne 0 ]; then
